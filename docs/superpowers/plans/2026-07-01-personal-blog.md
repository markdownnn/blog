...
// route param is named `route` (README convention). toUrlSlug lives in ../core/posts.
...
```
Note: 제목/설명 모두 ASCII 영어라 별도 한글 폰트 불필요. `getImageOptions`의 옵션 형태(`bgGradient`/`font`)가 설치된 `OGImageOptions` 타입과 안 맞으면 그 타입에 맞춰 최소 옵션(title/description)만 남겨도 됨.

- [ ] **Step 2: 엔드포인트 연결**

Create `src/pages/og/[...route].png.ts` (param 이름이 `route`여야 OGImageRoute의 getStaticPaths와 맞음):
```ts
